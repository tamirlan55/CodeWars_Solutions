...
class Song:
    ...
    def how_many(self, arr: list):
        '''Функция возвращающая количество новых слушателей'''
        temp_arr = []
        for elem in arr:
            if elem.lower() not in self.listener:
                self.listener.append(elem.lower())
                temp_arr.append(elem.lower())
        return len(temp_arr)
